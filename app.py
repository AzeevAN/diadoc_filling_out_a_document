...
def main(data_dict_json):
    ...
    elem = driver.find_element(by=By.CSS_SELECTOR, value='[type="password"]')
    ...
    elem_new_doc = driver.find_element(By.LINK_TEXT, 'Новый документ')
    elem_new_doc.click()
    sleep(3)

    # elem_upd = driver.find_element(By.XPATH, '/html/body/div[1]/div[2]/div/div[4]/div/div[2]/div[1]/div[2]/div[2]/span[4]/button/div/span')
    elem_upd = driver.find_element(By.XPATH, '/html/body/div[7]/div[2]/div/div[4]/div/div[2]/div[1]/div[2]/div[2]/span[4]/button/div/span')
    elem_upd.click()
    sleep(3)
    ...
    print("Нужно добавить колонку с артикулом / кодом товара")

    while True:
        stop_number = input('Для продолжение введите 0 :')
        if stop_number == '0':
            break
    ...
    for elem_r in elems_row:
        ...
        try:
            ...
            vendor_code = elem_r.find_element(By.CSS_SELECTOR, value='[data-tid="VendorCodeCell"]')
            input_vendor_code = vendor_code.find_element(By.TAG_NAME, value='input')
            input_vendor_code.send_keys(f"{data_dict_json[iter].get('barcode')}")
...
def go_to_button_add(driver: webdriver, position: int) -> webdriver:
    ...
    elem_new_row = driver.find_element(By.XPATH, '/html/body/div[7]/div[1]/div/span/div[2]/div[1]/div[8]/div/div/div[1]/div[2]/div/div/div[1]/div/div/div[1]/div/div/div[2]/div/span[2]/a')
    return elem_new_row
